...
def sns_topic_creator(request, thread_list):
    ...
    try:
        ...
        sns.topic_subscribe(f"BidNotificationForPropertyId{thread_list[0]}", user_email)
        thread_list[1] = True
...
def bid_creator(request, thread_list):
    try:
        ...
        if Bid.objects.filter(property=Property.objects.get(pk=thread_list[1])).filter(user = request.user):
            # Get the users existing bid and update with new bid value
            try:
                # The property id will always be index 1 in the list parameter
                bid = Bid.objects.get(property=Property.objects.get(pk=thread_list[1]), user = request.user)
            ...
            try:
                ...
                Bid.objects.create_bid(Property.objects.get(pk=thread_list[1]), request.user, float(request.POST.get('bid_amount')))
                thread_list[0] = True
            except Exception as e:
                print(e)
    except Exception as e:
